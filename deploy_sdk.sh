...
if [ -z "${RUNNING_DEPL}" ]; then
	...
	OLDEST_DB_CONTAINER="${MYSQL_CONTAINER_NAME}-${CT_LOWER_INDEX}"
...
if [ $RET_CODE -ne 200 ]; then 
	...
	echo " Service available at URL: https://${DEPLOYMENT_URL}//web/admin/install"
	exit 0
fi
...
#	echo "ssh -o "StrictHostKeyChecking no" -i  /tmp/sshkey.pem ${SSH_USER}@${DEPLOYMENT_SERVER} " sudo docker stop ${OLDEST_DB_CONTAINER} ${OLDEST_CONTAINER}"
        echo $RCMD | bash
	sleep 12
